...
try {
    New-Item -ItemType Directory -Force -Path 'builds/PocketCircuit_v0.2.1-dev' | Out-Null
    ...
    $exported = Join-Path $projectDirectory 'builds/PocketCircuit_v0.2.1-dev/PocketCircuit.exe'
    ...
    Copy-Item -LiteralPath 'tools/PLAYER_README.txt' -Destination 'builds/PocketCircuit_v0.2.1-dev/README.txt'
    Copy-Item -LiteralPath 'tools/GODOT_LICENSE.txt' -Destination 'builds/PocketCircuit_v0.2.1-dev/GODOT_LICENSE.txt'
    Compress-Archive -Path 'builds/PocketCircuit_v0.2.1-dev/*' -DestinationPath 'builds/PocketCircuit_v0.2.1-dev.zip' -Force
    Write-Output "Built: $projectDirectory\builds\PocketCircuit_v0.2.1-dev.zip"
} finally {
    Pop-Location
}
