...
try {
    New-Item -ItemType Directory -Force -Path 'builds/PocketCircuit_v0.1-dev' | Out-Null
    ...
    Run-Godot '--headless --path . --log-file builds/physics.log -- --autoplay-test' 'builds/physics.log'
    if (-not (Select-String -Path 'builds/physics.log' -SimpleMatch 'LIVE PHYSICS TEST: PASS')) { throw 'Physics test did not complete.' }
    Run-Godot '--headless --path . --export-release "Windows Desktop" --log-file builds/export.log' 'builds/export.log'
    $exported = Join-Path $projectDirectory 'builds/PocketCircuit_v0.1-dev/PocketCircuit.exe'
    ...
    Copy-Item -LiteralPath 'tools/PLAYER_README.txt' -Destination 'builds/PocketCircuit_v0.1-dev/README.txt'
    Copy-Item -LiteralPath 'tools/GODOT_LICENSE.txt' -Destination 'builds/PocketCircuit_v0.1-dev/GODOT_LICENSE.txt'
    Compress-Archive -Path 'builds/PocketCircuit_v0.1-dev/*' -DestinationPath 'builds/PocketCircuit_v0.1-dev.zip' -Force
    Write-Output "Built: $projectDirectory\builds\PocketCircuit_v0.1-dev.zip"
} finally {
    Pop-Location
}
